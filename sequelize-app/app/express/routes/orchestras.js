...
async function create(req, res) {
	if (req.body.id) {
		...
		const orchestra = await models.orchestra.create(req.body);
		res.status(201).send(orchestra);
	}
};
...
async function update(req, res) {
	...
	if (req.body.id === id) {
		const orchestra = await models.orchestra.update(req.body, {
			where: {
				id: id
			}
		});
		res.status(200).send(orchestra);
	} else {
